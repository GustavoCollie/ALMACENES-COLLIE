...
export const ProductForm = ({ onSubmit, onClose, initialData, inline = false }) => {
    ...
    const headerContent = (
        <div className="px-4 md:px-8 py-4 md:py-6 flex items-center justify-between border-b border-[#dadce0] bg-white sticky top-0 z-10">
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    {!isEdit && (
                        <p className="hidden md:block text-xs text-[#5f6368] mt-0.5 font-['Outfit']">
                            El stock inicial será 0 por defecto
                        </p>
                    )}
                </div>
            </div>
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4 bg-[#202124]/40 backdrop-blur-[2px] animate-fade-in modal-responsive">
            <div className="w-full max-w-[560px] bg-white md:rounded-2xl shadow-2xl overflow-hidden animate-scale-in flex flex-col h-full md:h-auto max-h-none md:max-h-[90vh]">
                {headerContent}
                {formBody}
            </div>
        </div>
